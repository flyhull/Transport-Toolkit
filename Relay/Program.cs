...
/// <summary>
/// Relay - Receive time based encrypted messages interlaced into generated png images from the signalr hub, recover the cyphertext from the images, decrypt it using using time based decryption, re-encrypt it using time based encryption and a different key, interlace the new cyphertext into a generated png image send it to a different signalr hub after a random delay
/// </summary>
/// 
...
builder.Services.AddTransient<IUsage, RelayRole>();
...
builder.Services.AddTransient<IFileProcessor, RelayFileProcessor>();
...
using (logger.BeginScope(nodeName + " Startup"))
{
    ...
    logger.LogInformation("THIS WINDOW WILL SHOW MESSAGES BEING TRANSFORMED AS THEY ARE PASSED BETWEEN SIGNALR HUBS");
